...
The trieff package offers two different bootstrapping methods, each with its own strengths:
...
- **Standard Bootstrap**: Use when cases can be considered independent or when computational efficiency is a priority (it is about 6 times faster).

- **Segment Bootstrap**: Use when temporal patterns and dependencies between concurrent patients are important. This method is particularly valuable for:
  * Data with strong temporal variations (e.g., day/night differences)
  * Analysis of systems where queue dynamics significantly impact patient flow
  * Studies focusing on crowding or surge effects

We recomend that segment bootstrapping should be the standard when looking at Emergency department data.

## Comparing Bootstrap Methods

The two methods often produce similar confidence intervals with segment bootstrapping often giving slightly wider intervals but somtimes slighly narrower, however the segment version is theoretically more valid based on assumptions of independence of data, which queue data cannot be said to have.
