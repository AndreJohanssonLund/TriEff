...
The trieff package uses simulation to calculate Theoretical Triage Effectiveness (TTE) by modeling how patients would be processed if strictly following their assigned priority levels. This vignette explains how the simulation works and demonstrates its efficient implementation.
...
The default simulation (`sim_te`) focuses only on segments containing time-critical (LOSET-positive) cases. This is due to that TE calculations are based on the mean wait of LOSET-positive devided by the mean wait time that we extract from the observed wait times - which we show further down is identical. 
...
Notice that non-LOSET cases may have NA theoretical wait times - this is expected based on that they were part of a que-segment with no LOSET-positive cases.
...
te_opt <- calc_te(simulated_data, overall_only = TRUE)
te_full <- calc_te(full_sim_data, overall_only = TRUE)
